...
describe('Test job lists result', function () {
	...
	before(function (done) {
		chai.request("http://ideelbackup.azurewebsites.net/")
			.get("/api/jobs")
	...
	it('The first entry in the array has known properties', function () {
		...
		expect(requestResult[0]).to.have.all.keys(['_id', 'city', 'description', 'jobID', 'salary', 'title', 'availability', 'companyName', '__v', 'businessID', 'phoneNo', 'startDate', 'endDate', 'address', 'appliedList', 'jobTags', 'watchingList', 'zipcode']);
...
describe('Test retrieve job where jobID=0 result', function () {
	...
	it('The first entry in the array has known properties', function () {
		expect(requestResult).to.have.all.keys(['_id', 'city', 'description', 'jobID', 'salary', 'title', 'availability', 'companyName', '__v', 'businessID', 'phoneNo', 'startDate', 'endDate', 'address', 'appliedList', 'jobTags', 'watchingList', 'zipcode']);
		expect(response.body).to.not.be.a.string;
	});
...
describe('Test bUsers lists result', function () {
	...
	it('The first entry in the array has known properties', function () {
		expect(requestResult[0]).to.have.all.keys(['_id', 'businessID', 'firstName', 'lastName', 'email', 'address', 'zipcode', 'jobTags', '__v', 'companyName', 'description', 'companyLicenseNo', 'username', 'password', 'pictureID','subscribers','tags','employees',]);
		expect(response.body).to.not.be.a.string;
	});
